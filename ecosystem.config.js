...
// IMPORTANT: This file is deployed ONCE to a stable path on the server
// (/var/www/portfolio/ecosystem.config.js) — it does NOT live inside a
// per-release folder. Both deploy-frontend.yml and deploy-backend.yml
// invoke it by that absolute path, e.g.:
//
//   pm2 reload /var/www/portfolio/ecosystem.config.js --only portfolio-frontend --env production
//
// Because of that, __dirname here is NOT the release directory, so paths
// below point at the *-current symlinks that each pipeline's deploy step
// re-points on every successful release (and rolls back on failure):
//
//   /var/www/portfolio/backend-current  -> frontend-releases/<tag>/  (Backend)
//   /var/www/portfolio/frontend-current -> frontend-releases/<tag>/ (Frontend)
const PORTFOLIO_ROOT = "/var/www/portfolio";
const BACKEND_DIR  = path.join(PORTFOLIO_ROOT, "backend-current");
const FRONTEND_DIR = path.join(PORTFOLIO_ROOT, "frontend-current");
...
module.exports = {
  apps: [
    ...
    // ── Backend (Express) ───────────────────────────────────────────────────
    {
      ...
      // Entry point. package.json's "main" is "server.js" at the Backend
      // root (not Backend/src/server.js) — keep this in sync with that.
      script: path.join(BACKEND_DIR, "server.js"),
